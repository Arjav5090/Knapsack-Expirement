...
const ResultSchema = zod_1.default.object({
    participantId: zod_1.default.string().min(1),
    score: zod_1.default.number(),
    passed: zod_1.default.boolean(),
    feedback: zod_1.default.string().optional(),
});
// REGISTER a new participant
exports.router.post('/api/v1/register', async (req, res) => {
    const id = crypto_1.default.randomUUID();
    const newDoc = await Participant_1.ParticipantModel.create({
        participantId: id,
        createdAt: new Date(),
    });
...
exports.router.post('/api/v1/register-prolific', async (req, res) => {
    ...
    const existingParticipant = await Participant_1.ParticipantModel.findOne({
...
exports.router.post('/api/v1/ingest-phase', async (req, res) => {
    ...
    try {
        ...
        const updated = await Participant_1.ParticipantModel.findOneAndUpdate({ participantId }, {
            $set: {
...
// SET FINAL RESULT
exports.router.post('/api/v1/set-result', async (req, res) => {
    const parsed = ResultSchema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ error: parsed.error.flatten() });
    const { participantId, score, passed, feedback } = parsed.data;
    const doc = await Participant_1.ParticipantModel.findOneAndUpdate({ participantId }, { $set: { result: { score, passed, feedback } } }, { new: true });
    if (!doc)
        return res.status(404).json({ error: 'Participant not found' });
...
exports.router.get('/api/v1/export-prolific-data', async (req, res) => {
    try {
        const participants = await Participant_1.ParticipantModel.find({
...
exports.router.get('/api/v1/study-stats', async (req, res) => {
    try {
        const totalParticipants = await Participant_1.ParticipantModel.countDocuments({
            'prolificData.prolificPid': { $exists: true }
        });
        const completedParticipants = await Participant_1.ParticipantModel.countDocuments({
            'prolificData.prolificPid': { $exists: true },
            'prolificData.completedAt': { $exists: true }
        });
        const phaseStats = await Participant_1.ParticipantModel.aggregate([
